...
//4. get the value of the input with jQuery. target the input id .val
	...
	$('#submit-btn').click(function(event) {
		...
		if (radioSelect === "eggs") {
		$('.modal-body').text(convertToEggs(userInput, substitutes));
		} else if (radioSelect === "milk") {
		// ELSE IF
		$('.modal-body').text(convertToButter(userInput, substitutes));
		} else {
		// ELSE
		$('.modal-body').text(convertToMilk(userInput, substitutes));
